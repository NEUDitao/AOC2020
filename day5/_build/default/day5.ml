...
let find_missing_id codes =
  ...
  let rec find_missing codes next =
    match codes with
    | [] -> raise (Failure "oh no")
    | first::rest -> print_int first; print_int next; print_endline ""; if first != next then first else
      find_missing rest (next + 1)
    in
...
let () =
  ...
  print_endline ("part 2: "^string_of_int(find_missing_id (List.map find_seat_id l)));;
